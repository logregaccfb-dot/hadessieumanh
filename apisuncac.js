...
// FORMAT DATA
// ======================================================
// Nguồn mới (wtxmd52) trả về { list: [ {id, _id, resultTruyenThong, dices: [d1,d2,d3], point}, ... ] }
// mới nhất ở đầu mảng, lưu tối đa 105 phiên.
// Vẫn hỗ trợ ngược định dạng cũ { data: [ {phien, Xuc_xac_1/2/3, Tong, Ket_qua}, ... ] } phòng khi đổi nguồn.
...
function normalizeData(data) {
    ...
    let normalized = data.map(item => {
        const dices = Array.isArray(item.dices) ? item.dices : null;
        const d1 = dices ? dices[0] : (item.Xuc_xac_1 ?? item.xuc_xac_1 ?? item.x1 ?? 0);
        const d2 = dices ? dices[1] : (item.Xuc_xac_2 ?? item.xuc_xac_2 ?? item.x2 ?? 0);
        const d3 = dices ? dices[2] : (item.Xuc_xac_3 ?? item.xuc_xac_3 ?? item.x3 ?? 0);
        const tong = item.point ?? item.Tong ?? item.tong ?? item.total ?? (d1 + d2 + d3);
        const ketQuaRaw = item.resultTruyenThong ?? item.Ket_qua ?? item.ket_qua ?? item.result ?? (tong >= 11 ? "tài" : "xỉu");
        const ketQuaUpper = ketQuaRaw.toString().toUpperCase();
        const ketQua = (ketQuaUpper === "TAI" || ketQuaUpper === "TÀI") ? "tài" : "xỉu";
        const phien = item.id ?? item.phien ?? item.Phien ?? item.session ?? 0;
        return {
            phien: phien,
            x1: d1, x2: d2, x3: d3,
    ...
    // (nguồn này lưu tới 105 phiên, và KHÔNG được cắt bớt để thuật toán "học" hết cả 105 phiên đó).
...
app.get("/taixiu", async (req, res) => {
    try {
        ...
        const dataArray = rawData.list || rawData.data || rawData || [];
